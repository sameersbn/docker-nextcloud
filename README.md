...
```bash
docker pull sameersbn/nextcloud:9.0.53-1
```
...
docker run --name nextcloud -itd --restart=always \
  ...
  sameersbn/nextcloud:9.0.53-1 app:nextcloud
```
...
docker run --name nextcloud-nginx -itd --restart=always \
  ...
  --publish 10080:80 \
  sameersbn/nextcloud:9.0.53-1 app:nginx
```
...
docker run --name nextcloud -it --rm [OPTIONS] \
  sameersbn/nextcloud:9.0.53-1 app:backup:create
```
...
```bash
docker run --name nextcloud -it --rm [OPTIONS] \
  sameersbn/nextcloud:9.0.53-1 app:backup:restore
```
...
```bash
docker run --name nextcloud -it --rm [OPTIONS] \
  sameersbn/nextcloud:9.0.53-1 app:backup:restore BACKUP=1417624827_nextcloud_backup.tar
```
